...
Most of the tested algorithms have the same performance as random guessing would, an AUC score of $0.5$.
CHAODA, though, performs much worse than random.
This probably because of how this specific dataset was collected, as opposed to the rest of the datasets.
...
The six attributes in this dataset each have a very narrow range to be in the Normal class, but can have a very varied range in the Abnormal class.
This causes the instances in the Normal class to clump up together, while the instances in the Abnormal class remain distance from each other.
Since the CHAODA methods rely on clusters as the substrate, they assign low scores to the instances in the Normal class, i.e.\ the outliers, and high scores to the Abnormal class, i.e.\ the inliers.
This curiosity could be remidied by a supervised version of CHAODA.

...
If a method exceeded 10 hours on a dataset, we terminated it and marked the corresponding cell with ``\textit{TO}''.
...
While we used the provided wrappers for each method we compared against, those methods are often implemented in C/C++.
...
\caption{Performance (AUC) of CHAODA vs. other methods on the 18 test datasets.}
...
% We considered comparisons against REPEN~\cite{pang2018learning} and RDP~\cite{wang2019unsupervised}, but REPEN's publicly available source code lacks information about dependency versions, and RDP took prohibitively long to train.

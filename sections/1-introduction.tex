...
% TODO NOAH explicitly point out that while other clustering approaches could be used, the novelty in CLAM is its potential genericity over all distance metrics, coupled with a built-in calculation (during clustering, and preserved for future use) of geometric and topological properties that are useful to the CHAODA algorithms.
% Also, time complexity of clustering is O(n lg n) while most others are O(n^2)

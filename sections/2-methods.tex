...
However, these notions help to build intuition and to aid understanding.
...
Given that the key to an effective ensemble method is that each member contributes a unique inductive bias~\cite{chen2017outlier}, we also note the intuition behind each algorithm's contributions to the ensemble.
...
This lets us combine scores into an ensemble (see Section~\ref{subsec:methods:ensemble}).
See~\cite{kriegel2011interpreting} for a more thorough discussion of anomaly-score normalization in ensemble methods.
...
The time complexity is $\mathcal{O}(|V|)$ because it requires a single pass over the clusters in a graph.

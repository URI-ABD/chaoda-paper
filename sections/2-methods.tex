...
This is essentially a form a transfer learning: we will learn such functions from a set of datasets that is \emph{completely distinct from} the set of datasets we use for benchmarking.
...
We normalize to the range $[0, 1]$, using gaussian normalization by default as defined in~\ref{alg:normalization}.
This normalization method is described in more detail in~\cite{kriegel2011interpreting}.
...
All benchmarks were conducted on an Intel Xeon E5-2690 v4 2.60GHz with 28 cores, with 512GB RAM and CentOS 7 Linux with kernel 3.10.0-1127.13.1.el7.x86\_64 \#1 SMP.

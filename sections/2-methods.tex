...
\end{itemize}
% TODO: is this really what we generalize to? Najib: PC ratios and EMAs of them are the six values you see in that linear_regression.csv file. They are the features we use. We can add more if we can think of proper features.
...
along with the exponential moving averages of these ratios.
...
% Then, this section only needs to contain the frilly details, like what made it into the outlier bin. Najib: yeah that's definitely in the cards. For now, we are not limited on space so let's leave this as a low priority.

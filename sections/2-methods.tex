...
\end{itemize}
% TODO: is this really what we generalize to?
...
A Graph can be built from clusters at a fixed depth in the cluster-tree (a layer-graph), or can contain clusters from multiple different depths in the tree (an optimal-graph).
In this work we consider the cardinality of a graph to be \textit{vertex cardinality}, i.e.\ the number of clusters in the graph.
...
Area Under the Curve (AUC) Receiver Operating Characteristic (ROC) is often used to measure the performance of anomaly detectors;
we wish to choose clusters that maximize this measure.
We can accomplish this by learning a function that takes a cluster and predicts contribution to AUC by selecting it.
...
We generate CLAM Manifolds for these training datasets, use the Linear Regression model to learn from these datasets, and apply the results to an entirely different collection of datasets.

We generate the initial training data by considering layer-graphs built from clusters at a uniform depth in the tree.
For each such graph, we calculate the means of the cardinality ratio, radius ratio, and local fractal dimension ratio; along with the exponential moving averages of these ratios.
...
% TODO: This feels like it could be better summarized with a table. Dataset name; n-instances; n-anomalies; dims used; what it's derived from.
%   Then, this section only needs to contain the frilly details, like what made it into the outlier bin.

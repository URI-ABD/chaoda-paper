...
The ground-truth labels are only used in training for the training datasets;
The ground-truth for the test datasets is used purely for post-hoc performance calculations.
...
We generate a clustering for dataset, and start by inducing several uniform-depth graphs from the tree.
We apply each individual algorithm described in Section~\ref{subsec:methods:individual-algorithms} to each such graph and, using the ground-truth for the training dataset, we calculate the AUC for the subset of points in each cluster and use that AUC as the training target for our meta-ml models.
...
This is expected fraction of time that the cluster will be visited by a long random walk over the graph component.  % NOAH: citation perhaps? I don't know from where though
...
Its worst-case time complexity is $\mathcal{O}(|V|^{2.3728596})$ given by the matrix multiplication algorithm given in \dots % Noah: please cite https://arxiv.org/pdf/2010.05846.pdf

...
A corollary to this invariant is that the graph will never contain two vertices such that one vertex corresponds to a ancestor or descendent cluster of another vertex.
...
These algorithms will often also incorporate information from the tree, such as the parent-child ratios described in Section~\ref{subsubsec:methods:clam:clustering}.
...
Importantly, we will learn this function on datasets \emph{completely unrelated to} the ones on which we test;
we are not learning any dataset-specific parameters, but instead the geometric and topological properties of the data.
...
We normalize to the range $[0, 1]$, using gaussian normalization by default as described in~\ref{alg:ormalization}.

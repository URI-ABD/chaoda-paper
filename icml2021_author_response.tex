...
Reviewer 6 points out that we are using Anomaly and Outlier distinctly in the introduction, and interchangeably later.
\emph{We thank Reviewer 6 for this observation, and have added an explanation to the Introduction.}
...
Running all the methods we compare to on additional datasets will require two weeks of computation time; thus, it was not feasible in the one week given to respond to reviewers.
Additionally, many of these datasets contain a mix of categorical and numerical features, and do not represent a Banach space, so CLAM cannot currently cluster them.
\emph{Some additional comparisons are running and will be included in the camera-ready if the paper is accepted}.

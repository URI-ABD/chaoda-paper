...
We treat class 2 (lodgepole pine) as the inliers, and class 4 (conttonwood/willow) as the outliers.
...
Here we provide short proofs for the time-complexity of the CHAODA algorithms.
...
\subsection{Relative Component Cardianlity}

This method first finds the components of the graph.
This costs $\mathcal{O}(|E|)$ because we have to check each edge once.
The cardinality of each component is cached when traversing the clusters to find find components.
...
This algorithm performs a linear pass over the clsuters in the graph and first computes the eccentricity of each cluster.
Finding the eccentricity of a vertex in a graph is worst-case $\mathcal{O}(|E|)$ when the graph consists of a single component.
This brings the total cost up to $\mathcal{O}(|E| \cdot |V|)$.
Next, the algorithm performs a traversal from each cluster.
This adds a constant factor of $\mathcal{O}(|E|)$ to the cost, and can be ignores.
The total time-complexity of this algorithm is thus $\mathcal{O}(|E| \cdot |V|)$.

\subsection{Child-Parent Cardinality Ratio}

While building the tree with CLAM, we cache the child-parent cardinality ratios of every cluster, because it proved useful for purposes other than anomaly deetection.
...
We set the transition probability from a cluster to a neighbor to be inversely proportional to the distance between their centers, normalized by all possible neighbors of the clsuter.
...
Thus, the average performance is much better than that suggested by the quadratic space time-complexity.
...
Also just as with Random Walks, because $|V| \ll n$ and graphs only rarely consist of only one component, the average-case performance is much better.
...
% TODO: Add more images here and discuss UMAPS
...
\begin{figure*}
   ...
   \caption{UMAP projections of Cardio (first row), Optdigits (second row), Mnist (third row) and Ionosphere (right).
   ...
   For OptDigits, thre are several distinct pieces to the manifold, perhaps corresponding to different digits.
